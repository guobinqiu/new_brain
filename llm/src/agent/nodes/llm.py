...
from llm.src.agent.llm import get_llm
from llm.src.agent.stream import safe_get_writer, write_reply
from llm.src.infra.logger import get_logger
from llm.src.infra.metrics import Timer, mark_llm_error, mark_llm_ok
...
def get_semaphore() -> asyncio.Semaphore:
    """获取并发 semaphore；未初始化时自动以默认值 10 创建。"""
    global _semaphore
    if _semaphore is None:
        import os
        limit = int(os.getenv("LLM_CONCURRENCY_LIMIT", "10"))
        _semaphore = asyncio.Semaphore(limit)
        logger.info("semaphore auto-initialized (fallback)", limit=limit)
    return _semaphore
