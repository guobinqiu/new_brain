...
def test_unstructured_runtime_config_uses_local_table_snapshot(monkeypatch, tmp_path):
    import sys
    import types
    ...
    model_config.write_text("{}", encoding="utf-8")
    hf_cache = tmp_path / "models" / "huggingface" / "hub"
    table_cache = hf_cache / "models--microsoft--table-transformer-structure-recognition"
    snapshot = table_cache / "snapshots" / "abc123"
    snapshot.mkdir(parents=True)
    (table_cache / "refs").mkdir()
    (table_cache / "refs" / "main").write_text("abc123", encoding="utf-8")

    tables = types.SimpleNamespace(DEFAULT_MODEL="microsoft/table-transformer-structure-recognition")
