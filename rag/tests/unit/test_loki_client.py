...
def test_loki_log_query_escapes_labels():
    from rag.loki_client import log_query, trace_query
...
def test_loki_parse_logs_returns_rows_in_time_order():
    from rag.loki_client import parse_logs

    rows = parse_logs([
        {"stream": {"node_id": "node-1", "container": "rag-api"}, "values": [
...
def test_loki_timestamp_to_ns_accepts_ui_milliseconds():
    from rag.loki_client import timestamp_to_ns
