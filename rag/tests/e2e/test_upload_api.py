...
class TestUploadAPI:
    def test_upload_txt_stores_object(self, app_api_client, api_client, test_txt_path, monkeypatch):
        ...
        with open(test_txt_path, "rb") as f:
            resp = api_client.post(
                "/api/open/rag/upload", data={"app_id": app_api_client.app_id}, files={"file": ("test_ai.txt", f, "text/plain")}
            )
    ...
    def test_upload_image_png_stores_object(self, app_api_client, api_client, test_img_path, monkeypatch):
        ...
        with open(test_img_path, "rb") as f:
            resp = api_client.post(
                "/api/open/rag/upload", data={"app_id": app_api_client.app_id}, files={"file": ("test_ocr.png", f, "image/png")}
            )
        assert resp.status_code == 200, resp.text
        data = resp.json()
    ...
    def test_upload_xlsx_stores_object(self, app_api_client, api_client, tmp_path, monkeypatch):
        """``POST /api/open/rag/upload`` stores a .xlsx file in object storage."""
        ...
        with open(xlsx_file, "rb") as f:
            resp = api_client.post(
                "/api/open/rag/upload", data={"app_id": app_api_client.app_id}, files={"file": ("table.xlsx", f, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")}
            )
        assert resp.status_code == 200, resp.text
        data = resp.json()
    ...
    def test_index_presigned_url_generates_file_id_for_app(self, app_api_client, test_txt_path, monkeypatch):
        """``POST /api/open/rag/files`` generates the indexed file_id."""
    ...
    def test_async_index_job_returns_file_id(self, app_api_client, monkeypatch):
        """``POST /api/open/rag/files/jobs`` enqueues an async index job and returns file_id."""
