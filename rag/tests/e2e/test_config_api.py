...
class TestConfigAPI:
    ...
    def test_get_config_includes_components(self, api_client):
        ...
        assert cfg["store"]["type"]
        assert cfg["dense"]["name"]
        assert cfg["dense"]["model_name"]
    ...
    def test_get_config_includes_runtime_switchable_components(self, api_client):
        ...
        assert all("active" in item for item in available["ocr"])
...
class TestMonitorAPI:
    def test_monitor_returns_runtime_data_and_index_contract(self, app_api_client, api_client, test_txt_path):
        ...
        assert monitor["profile"]["store"]["type"]
        components = {component["name"]: component for component in monitor["components"]}
        assert components["Store"]["status"] == "ready"
        assert components["Store"]["model"] == monitor["profile"]["store"]["type"]
        assert components["Dense"]["status"] == "ready"
        assert components["Dense"]["model"] == monitor["profile"]["dense"]["model_name"]
        if monitor["profile"]["sparse"] is None:
            assert components["Sparse"]["status"] == "disabled"
            assert components["Sparse"]["model"] is None
        else:
            assert components["Sparse"]["status"] == "ready"
            assert components["Sparse"]["model"] == monitor["profile"]["sparse"]["name"]
        assert components["Rerank"]["status"] in {"disabled", "ready"}
        if monitor["profile"]["ocr"] is None:
            assert components["OCR"]["status"] == "disabled"
            assert components["OCR"]["model"] is None
        else:
            assert components["OCR"]["status"] == "ready"
            assert components["OCR"]["model"] == monitor["profile"]["ocr"]["model_name"]
        ...
        expected_modes = ["dense", "sparse", "hybrid"] if monitor["profile"]["sparse"] is not None else ["dense"]
        assert monitor["capabilities"]["search_modes"] == expected_modes
