...
def _keep_request_log(record: logging.LogRecord) -> bool:
    if record.levelno >= logging.WARNING:
        return True
    if record.name == "brain.trace" and getattr(record, "event", None) == "service_request":
        return getattr(record, "path", None) != "/ready"
    if not isinstance(record.args, tuple):
        return True
    if record.name == "uvicorn.access" and len(record.args) == 5:
        return urlsplit(str(record.args[2])).path != "/ready"
    if record.name == "httpx" and len(record.args) == 5:
        return urlsplit(str(record.args[1])).path != "/ready"
    return True


def install_ready_log_filter() -> None:
    for name in ("uvicorn.access", "httpx", "brain.trace"):
        logging.getLogger(name).addFilter(_keep_request_log)
...
def configure_logging(config: LoggingConfig, stream: TextIO | None = None) -> None:
    ...
    for logger_name in ("brain.trace", "services.rag", "services.rag.trace", "services.parser", "services.inference", "uvicorn", "uvicorn.error", "uvicorn.access"):
