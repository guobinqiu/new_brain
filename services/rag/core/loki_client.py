...
def container_service(container: str) -> str:
    return re.sub(r"\.[0-9]+\.[a-z0-9]{25}$", "", container)


def log_query(node_id: str | None = None, container: str | None = None, *, include_tasks: bool = False) -> str:
    labels = []
    if node_id:
        labels.append(f'node_id="{_escape_label(node_id)}"')
    if container:
        if include_tasks:
            pattern = re.escape(container) + r"(\.[0-9]+\.[a-z0-9]{25})?"
            labels.append(f'container=~"{_escape_label(pattern)}"')
        else:
