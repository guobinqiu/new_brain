...
def test_parse_peers_trims_empty_items():
    from services.rag.core.nodes import parse_peers
...
def test_fetch_peers_returns_success_and_forwards_authorization():
    from services.rag.core.nodes import fetch_peers

    async def _run():
        client = FakeAsyncClient(
            {
                "http://a:8000/api/rag/config": FakeResponse(200, {"node_id": "node-a", "ready": True}),
                "http://b:8000/api/rag/config": FakeResponse(200, {"ready": True}),
            }
        )

        rows = await fetch_peers(["http://a:8000", "http://b:8000"], "/api/rag/config", "Bearer token", client=client)
...
def test_fetch_peers_keeps_failed_peer_as_unreachable():
    ...
    async def _run():
        client = FakeAsyncClient(
            {
                "http://a:8000/api/rag/config": FakeResponse(500, {"error": "failed"}),
                "http://b:8000/api/rag/config": httpx.ConnectTimeout("timeout"),
            }
        )

        rows = await fetch_peers(["http://a:8000", "http://b:8000"], "/api/rag/config", None, client=client)
...
def test_local_result_uses_env_node_id(monkeypatch):
    from services.rag.core.nodes import local_result

    monkeypatch.setenv("HOSTNAME", "node-local")
