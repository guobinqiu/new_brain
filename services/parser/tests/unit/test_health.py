...
from services.parser.app.main import ParseFileResponse, _block_response, app, health, parse_file, ready
...
def test_parse_file_returns_blocks(monkeypatch):
    ...
    file = UploadFile(filename="sample.txt", file=BytesIO(b"hello parser"))

    response = parse_file(file)
