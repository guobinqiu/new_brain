...
from services.llm.src.agent.stream import safe_get_writer
...
async def rag_prefetch_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    from services.llm.src.api.auth import get_current_credential
    ...
    try:
        ...
        req = SearchRequest(query=query, top_k=3)
