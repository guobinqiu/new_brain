...
# 任务：让 YAML 配置（→ settings.rag_max_retries → RagClient.max_retries）
...
def test_rag_retry_resolves_rag_max_retries_from_settings(monkeypatch):
    ...
    from services.llm.src.infra.retry import _reset_rag_retry_for_tests, _resolve_rag_policy
...
def test_rag_retry_policy_zero_retries_means_one_attempt(monkeypatch):
    ...
    from services.llm.src.config import settings
    from services.llm.src.infra.retry import _resolve_rag_policy
...
async def test_rag_retry_respects_settings_max_retries(monkeypatch):
    ...
    """
    from services.llm.src.config import settings
    from services.llm.src.infra.retry import _get_rag_retry, _reset_rag_retry_for_tests, _resolve_rag_policy
