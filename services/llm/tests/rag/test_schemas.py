...
def test_search_request_exclude_none_drops_optional_fields():
    ...
    cls = _try_import_symbol("services.llm.src.rag.schemas", "SearchRequest")
    if cls is None:
...
def test_document_parses_minimal_fields():
    ...
    Document = _try_import_symbol("services.llm.src.rag.schemas", "Document")
    if Document is None:
...
def test_response_accepts_result_without_score():
    ...
    Response = _try_import_symbol("services.llm.src.rag.schemas", "SearchResponse")
    Document = _try_import_symbol("services.llm.src.rag.schemas", "Document")
...
def test_document_parses_simplified_qdrant_open_search_result():
    """Document 应兼容 qdrant `/api/v1/rag/search` 的简化 result 结构。"""
    Document = _try_import_symbol("services.llm.src.rag.schemas", "Document")
    if Document is None:
        pytest.fail("rag.schemas.Document not implemented — RED")
    raw = {
...
def test_schemas_module_path_is_rag_schemas():
    ...
    try:
        from services.llm.src.rag.schemas import (  # type: ignore  # noqa: F401
            Document,
            SearchRequest,
            SearchResponse,
        )
