...
logger = logging.getLogger("services.inference.providers.tei")
...
class TeiModel:
    ...
    def _log_call(self, operation: str, started: float, response: httpx.Response | None, error: UpstreamServiceError | None = None, **context) -> None:
        logger.log(
            ...
            f"TEI {operation} request {'failed' if error else 'completed'}",
            exc_info=error is not None,
            extra={
                "event": f"tei_{operation}_{'error' if error else 'success'}",
                "service": "inference",
                "operation": operation,
                "model": self.model,
