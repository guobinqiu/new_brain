...
class MarkdownBlockParser(BlockParser):
    ...
    def parse(self, filepath: str, ocr: OCR | None = None) -> list[Block]:
        ...
        while index < len(lines):
            if self._is_table_start(lines, index):
                self._flush_text_buffer(blocks, text_buffer)
                table_lines = [lines[index], lines[index + 1]]
                index += 2
                while index < len(lines) and self._is_table_row(lines[index]):
                    table_lines.append(lines[index])
                    index += 1
                blocks.extend(table_html_to_blocks(self._table_lines_to_html(table_lines), self.parser_config))
                continue

            normalized = self._normalize_text_line(lines[index])
            if normalized:
    ...
    def _flush_text_buffer(self, blocks: list[Block], text_buffer: list[str]) -> None:
    ...
    def _is_table_start(self, lines: list[str], index: int) -> bool:
        return index + 1 < len(lines) and self._is_table_row(lines[index]) and self._is_table_separator(lines[index + 1])
    ...
    def _table_lines_to_html(self, lines: list[str]) -> str:
        rows = [self._split_table_row(line) for offset, line in enumerate(lines) if offset != 1]
        rendered_rows = []
        for row in rows:
