...
class TestFilesAPI:
    ...
    def test_list_files_api(self, app_api_client, api_client, monkeypatch):
        ...
        main.application.database.upsert_file(app_api_client.app_id, "file-a", "test_ai.txt", f"s3://rag/uploads/{app_api_client.app_id}/file-a/test_ai.txt", size=7)
    ...
    def test_delete_file_api_keeps_index_delete_when_storage_delete_fails(self, app_api_client, api_client, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr(main, "_delete_index_file", lambda file_id, principal: calls.append(("index", file_id, principal.app_id)) or {"deleted_chunks": 2})
        monkeypatch.setattr(main, "_delete_storage_file", lambda app_id, file_id: (_ for _ in ()).throw(RuntimeError("minio down")))
    ...
    def test_delete_file_api_removes_indexed_chunks_and_file_record(self, app_api_client, api_client, test_txt_path, monkeypatch):
        ...
        with main.application.store.app_context(app_api_client.app_id):
            assert main.application.store.get_total_chunks([file_id]) > 0
