...
class TestUploadAPI:
    def test_upload_txt_stores_object(self, app_api_client, api_client, test_txt_path, monkeypatch):
        """``POST /api/admin/upload`` stores a .txt file in object storage and returns s3_url."""
        import main
        ...
        with open(test_txt_path, "rb") as f:
            resp = api_client.post(
                "/api/admin/upload", data={"app_id": "imsdom"}, files={"file": ("test_ai.txt", f, "text/plain")}
            )
    ...
    def test_upload_unsupported_type(self, app_api_client, api_client, tmp_path):
        ...
        with open(bad, "rb") as f:
            resp = api_client.post(
                "/api/admin/upload", data={"app_id": "imsdom"}, files={"file": ("data.xyz", f, "application/octet-stream")}
            )
    ...
    def test_upload_image_png_stores_object(self, app_api_client, api_client, test_img_path, monkeypatch):
        """``POST /api/admin/upload`` stores a .png image file in object storage."""
        import main
        ...
        with open(test_img_path, "rb") as f:
            resp = api_client.post(
                "/api/admin/upload", data={"app_id": "imsdom"}, files={"file": ("test_ocr.png", f, "image/png")}
            )
        assert resp.status_code == 200, resp.text
        data = resp.json()
    ...
    def test_upload_no_filename(self, app_api_client, api_client, tmp_path):
        ...
        with open(bad, "rb") as f:
            resp = api_client.post(
                "/api/admin/upload", data={"app_id": "imsdom"}, files={"file": ("", f, "text/plain")}
            )
    ...
    def test_async_index_job_returns_job_id(self, app_api_client, monkeypatch):
        """``POST /api/index/jobs`` creates an async index job and returns job_id."""
        import main
    ...
    def test_index_presigned_url_accepts_caller_uuid_file_id(self, app_api_client, monkeypatch):
        """``POST /api/index`` accepts caller-provided UUID file_id."""
        import main
