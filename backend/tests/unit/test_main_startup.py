...
def test_lifespan_keeps_process_healthy_when_application_start_fails(monkeypatch):
    ...
    token = issue_token(failing_application.config.auth, Principal(type="admin", app_id=failing_application.config.auth.app.app_id))

    with TestClient(main.app) as client:
        health_resp = client.get("/api/health")
        search_resp = client.post("/api/search", json={"query": "test"}, headers={"Authorization": f"Bearer {token}"})
