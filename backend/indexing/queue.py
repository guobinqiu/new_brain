...
from indexing.repository import (
    DEFAULT_REDIS_URL,
    INDEX_QUEUE_NAME,
    IndexJobRepository,
    job_failure_ttl,
    ...
    try:
        ...
        try:
            return _celery_app().send_task(
                ...
                queue=queue_name,
                retry=True,
                retry_policy=_send_task_retry_policy(),
            )
        except Exception:
            repository.rollback_job(job_id, app_id=app_id)
            raise
...
def list_index_jobs(*, limit: int = 50, app_id: str | None = None) -> dict:
    from redis.exceptions import RedisError
    ...
    repository = IndexJobRepository(redis, queue_name)
    try:
        items = repository.list_jobs(limit=limit, app_id=app_id)
        celery = _celery_app()
        jobs = []
        for item in items:
...
def _send_task_retry_policy() -> dict[str, float | int]:
    return {
        "max_retries": int(os.getenv("INDEX_ENQUEUE_MAX_RETRIES", "3")),
        "interval_start": float(os.getenv("INDEX_ENQUEUE_RETRY_INTERVAL_START", "0")),
        "interval_step": float(os.getenv("INDEX_ENQUEUE_RETRY_INTERVAL_STEP", "0.2")),
        "interval_max": float(os.getenv("INDEX_ENQUEUE_RETRY_INTERVAL_MAX", "1")),
    }


def _job_result_ttl() -> int:
    return job_result_ttl()


def _job_failure_ttl() -> int:
    return job_failure_ttl()
