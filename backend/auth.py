...
def issue_token(config: AuthConfig, principal: Principal) -> str:
...
def verify_token(config: AuthConfig, token: str) -> Principal:
    parts = token.split(".")
    if len(parts) != 3:
...
def authenticate_password(config: AuthConfig, username: str | None, password: str | None) -> Principal:
...
def principal_from_authorization(config: AuthConfig, authorization: str | None) -> Principal:
